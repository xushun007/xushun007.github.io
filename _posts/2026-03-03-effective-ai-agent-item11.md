---
title: "《Effective AI Agent》条目 11: 为工具调用构建零信任的防御性校验"
author: xushun
date: 2026-03-03
categories: [Agent]
tags: [Agent, Effective AI Agent, Tool]
...
在 Agent 系统里，这类问题会被放大。LLM 生成的 tool args 本质上就是外部输入。不经校验就进入 Shell、支付，结果就不只是报错，而是故障、甚至资损。

## 原理分析

系统稳不稳，取决于你如何看待 tool args：

* 当作内部对象，就会默认它完整、正确、可信；
* 当作外部请求，才会去做结构校验、语义校验、权限校验和审计。
